...
import axios from "axios";
import Toast from './Toast';
import AdminProfileUpdate from './AdminProfileUpdate'; // Import the profile update component
...
const PatientNavbar = () => {
  ...
  useEffect(() => {
    ...
    if (id) {
      axios.get(`http://localhost:1225/patient/${id}`)
        .then((response) => {
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className={`w-${open ? "72" : "20"} bg-dark-purple h-full p-5 pt-8 relative duration-300`}>
        ...
        <ul className="pt-6">
          ...
          {/* <Link className="flex items-center gap-x-4 mt-10 p-2.5">
            <FaUser size={24} className="text-gray-300 size-5" onClick={openProfileModal} />
            <span className={`${!open && "hidden"} origin-left transition-transform duration-300 text-gray-300`} onClick={openProfileModal}>
              Profile
            </span>
          </Link> */}
          <Link className="flex items-center gap-x-4 mt-10 p-2.5">
          ...
          <div className="flex items-center space-x-6">
            ...
            <div className="relative">
              ...
              {profileOpen && (
                <div className="absolute top-full right-0 bg-dark-purple text-white border border-gray-200 rounded-lg shadow-lg w-48 mt-2">
                  <ul className="py-2">
                    <li className="px-4 py-2 hover:bg-light-gray cursor-pointer" onClick={openProfileModal}>
                      View Profile
                    </li>
                    <li className="px-4 py-2 hover:bg-light-gray cursor-pointer" onClick={handleLogout}>
                      Logout
