    ...
    <div className="p-4 ml-24">
              ...
              {records.map((record, rowIndex) => (
                <tr key={rowIndex} className="hover:bg-gray-100">
                  ...
                  <td className="py-3 px-4 border-b border-gray-200 text-center">
                    <button
                      onClick={() => openModal(record, 'patientHistory')}
                      className="text-blue-500 hover:text-blue-700"
                    >
                      View History
                    </button>
      ...
      {isModalOpen && (
        <Modal
          isOpen={isModalOpen}
          onClose={closeModal}
          title={modalType === 'patientHistory' ? 'Patient History' : 'Patient Info'}
          content={<pre className="whitespace-pre-wrap">{modalContent}</pre>}
        />
      )}
    </div>
  );
