...
const EHR = () => {
  ...
  const handleDownloadAsPDF = (fileData, fileName) => {
    const doc = new jsPDF();
    const content = Uint8Array.from(atob(fileData), c => c.charCodeAt(0));
    doc.text(new TextDecoder().decode(content), 10, 10);
    doc.save(fileName.replace('.txt', '.pdf')); // Ensure file extension is .pdf
  };
          ...
          {ehrRecords.map(record => (
            <div key={record.recordId} className="bg-white shadow-md rounded-lg p-6 flex flex-col border border-gray-200">
              ...
              {record.fileData && (
                <div className="flex space-x-4">
                  <button
                    onClick={() => handleViewInModal(record.fileData, `record_${record.recordId}.txt`)}
                    className="bg-green-600 text-white py-2 px-4 rounded-md shadow hover:bg-green-700 transition duration-300"
                  >
                    View File
                  </button>
                  <button
                    onClick={() => handleDownloadAsPDF(record.fileData, `record_${record.recordId}.pdf`)}
                    className="bg-blue-600 text-white py-2 px-4 rounded-md shadow hover:bg-blue-700 transition duration-300"
                  >
                    Download
                  </button>
